...
class Trainer():
    ...
    def train(self, num_epochs, optimizer, create_graph=False):
        ...
        for epoch in range(num_epochs):
            ...
            arrayHolder.append(running_loss / len(self.train_dataloader))

            #if create_graph:
                # Predict after each epoch of training
                
            train_preds, train_Y = self.predict(self.train_dataloader, train_set=True)
            val_preds, val_Y = self.predict(self.val_dataloader)
                
            train_acc = accuracy_score(train_Y, train_preds)
            val_acc = accuracy_score(val_Y, val_preds)
            print((epoch +1), "Train Acc: ", train_acc)
            print((epoch +1), "Val Acc: ", val_acc)

            results = confusion_matrix(val_Y, val_preds)
        ...
        blocks = np.array(arrayHolder)
        df['values'] = blocks.tolist()
        blocks1 = np.array(train_accs)
        blocks2 = np.array(val_accs)
        df['train'] = blocks1.tolist()
        df['val'] = blocks2.tolist()
    ...
    def predict(self, dataloader, train_set=False):
